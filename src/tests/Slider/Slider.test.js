...
describe('Test en <Slider />', () => {
	...
	test('componente <Slider /> recibe parametros correctamente', () => {
		...
		//Evaluar para chrome
		expect(wrapper).toHaveStyleRule('appearance', 'none', { modifier: '::-webkit-slider-thumb' });
		expect(wrapper).toHaveStyleRule('width', '25px', { modifier: '::-webkit-slider-thumb' });
		expect(wrapper).toHaveStyleRule('height', '25px', { modifier: '::-webkit-slider-thumb' });
		expect(wrapper).toHaveStyleRule('background', '#4caf50', { modifier: '::-webkit-slider-thumb' });
		expect(wrapper).toHaveStyleRule('cursor', 'pointer', { modifier: '::-webkit-slider-thumb' });
		expect(wrapper).toHaveStyleRule('transition', 'transform 0.15s ease-in-out', {
			modifier: '::-webkit-slider-thumb',
		});
		//Evaluar focus - chrome
		expect(wrapper).toHaveStyleRule('transform', 'scale(1.1)', { modifier: ':active::-webkit-slider-thumb' });
		...
		expect(wrapper).toHaveStyleRule('width', '25px', { modifier: '::-moz-range-thumb' });
		expect(wrapper).toHaveStyleRule('height', '25px', { modifier: '::-moz-range-thumb' });
		expect(wrapper).toHaveStyleRule('border-radius', '50%', { modifier: '::-moz-range-thumb' });
		expect(wrapper).toHaveStyleRule('background', '#4caf50', { modifier: '::-moz-range-thumb' });
		expect(wrapper).toHaveStyleRule('cursor', 'pointer', { modifier: '::-moz-range-thumb' });
		expect(wrapper).toHaveStyleRule('transition', 'transform 0.15s ease-in-out', {
			modifier: '::-moz-range-thumb',
		});
		//Evaluar focus - mozilla
		expect(wrapper).toHaveStyleRule('transform', 'scale(1.1)', { modifier: ':active::-moz-range-thumb' });
		expect(wrapper).toHaveStyleRule('box-shadow', '0 0 0 3px #d7dcdf, 0 0 0 6px #4caf50', {
    ...
    test('componente <Slider /> debe mostrar color correspondiente', () => {
		...
		//Chrome
		expect(wrapper).toHaveStyleRule('background', '#8B5CF6', { modifier: '::-webkit-slider-thumb' });
        expect(wrapper).toHaveStyleRule('box-shadow', '0 0 0 3px #d7dcdf, 0 0 0 6px #8B5CF6', {
			modifier: ':active::-webkit-slider-thumb',
		});
        //Mozilla
        expect(wrapper).toHaveStyleRule('background', '#8B5CF6', { modifier: '::-moz-range-thumb' });
		expect(wrapper).toHaveStyleRule('box-shadow', '0 0 0 3px #d7dcdf, 0 0 0 6px #8B5CF6', {
			modifier: ':active::-moz-range-thumb',
		});
	});
